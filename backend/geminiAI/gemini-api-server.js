#!/usr/bin/env node
/*
  Lightweight Node HTTP server that mirrors backend/geminiAI/gemini-api.php
  - Listens by default on port 8000
  - Supports GET (diagnostic), OPTIONS, and POST
  - Accepts requests to /gemini-api.php and /gemini-api
  - Uses global fetch (Node 18+) to call Google Generative Language API
  - Reads API key from environment variable GEMINI_API_KEY or falls back to the hardcoded key (for local testing)

  Run:
    node backend/geminiAI/gemini-api-server.js

  Or set key and custom port:
    GEMINI_API_KEY=your_key node backend/geminiAI/gemini-api-server.js 9000
*/
...
function generateLocalRecommendation(promptText) {
  // Very small heuristic parser to extract simple signals from the prompt text
  const txt = (promptText || '').toLowerCase();
  // Temps: find numbers with °C or 'entre X y Y'
  ...
  // Build recommendation avoiding the banned phrase
  const parts = [];
  if (isStorm || isRain) {
  ...
  // Keep 1-2 short sentences and end with required phrase
  let result = parts.join(' ');
  // Remove the banned phrase if present
...
if (!globalThis.fetch) {
  console.warn('global fetch not available in this Node runtime. Node 18+ is recommended.');
}

// Global error handlers to log uncaught exceptions and promise rejections
process.on('uncaughtException', (err) => {
  console.error('Uncaught exception in Gemini server:', err && err.stack ? err.stack : err);
});
process.on('unhandledRejection', (reason) => {
  console.error('Unhandled rejection in Gemini server:', reason && reason.stack ? reason.stack : reason);
});
...
async function handlePost(req, res) {
  try {
    ...
    // If provider is local or no apiKey provided, use heuristic generator
...
server.listen(DEFAULT_PORT, () => {
  console.log(`Gemini API server listening on http://localhost:${DEFAULT_PORT}`);
